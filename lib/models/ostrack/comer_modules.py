...
from .attention import FocusedLinearAttention
...
class MultiscaleExtractor(nn.Module):
    def __init__(self, dim, num_heads=6, split_size=7, qk_scale=None, attn_drop=0.,focusing_factor=3, kernel_size=5, resolution=7,
                 with_cffn=True, cffn_ratio=0.25, drop=0., drop_path=0.,
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), with_cp=False):
        super().__init__()
        self.query_norm = norm_layer(dim)
        self.feat_norm = norm_layer(dim)
        self.attn = FocusedLinearAttention(dim, resolution=resolution, idx=-1,
...
class CTI_toC(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        if cnn_feature_interaction:
            self.cfinter = MultiscaleExtractor(dim=dim, num_heads=num_heads,split_size=7, qk_scale=None, attn_drop=0.,focusing_factor=3, kernel_size=5, resolution=7,
    ...
    def forward(self, query,  feat, H, W):
        ...
        def _inner_forward(query, feat):
            ...
            if self.cnn_feature_interaction:
                query = self.cfinter(query=self.query_norm(query), feat=self.feat_norm(query))
...
class CTIBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 drop=0., drop_path=0., with_cffn=True, cffn_ratio=0.25, init_values=0.,split_size=7, qk_scale=None, attn_drop=0.,focusing_factor=3, kernel_size=5, resolution=7,
