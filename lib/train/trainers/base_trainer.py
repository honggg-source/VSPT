        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    save_epoch_interval = getattr(self.settings, "save_epoch_interval", 10)
                    save_last_n_epoch = getattr(self.settings, "save_last_n_epoch", 15)
                    # if epoch > (max_epochs - save_last_n_epoch) or epoch % save_epoch_interval == 0:
                    if epoch <= save_last_n_epoch or epoch % save_epoch_interval == 0:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                self.save_checkpoint()
                # 计算训练时间和GPU使用率
                train_time = time.time() - start
                print('trianing time : {} minutes'.format(train_time / 60.0))

                peak_memory = (torch.cuda.max_memory_allocated() / 1024 ** 2)
                print('GPU footprint : {} MiB'.format(peak_memory))
            except:
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        for key in fields:
            ...
            if key == 'net':
                ...
                # print("检查点中的参数组：", checkpoint_dict['optimizer']['param_groups'])
                # print("当前优化器的参数组：", self.optimizer.param_groups)
